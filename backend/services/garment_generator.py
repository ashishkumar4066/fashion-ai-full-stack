"""
Garment Generator service — generates garment product images from a text prompt.

Uses Gemini 2.5 Flash via PiAPI. Generated images are saved locally to data/garment/.
A JSON record for each generation is appended to data/garment/garment.json.
The output image URL can be passed directly to TryOnService as the garment_image input.

Usage:
    generator = GarmentGenerator()
    id, name, file_path, image_url = await generator.generate(
        prompt="blue denim jacket, front view",
        aspect_ratio="1:1",
    )
"""
...
GARMENT_OUTPUT_DIR = Path("data/garment")
GARMENT_REGISTRY   = GARMENT_OUTPUT_DIR / "garment.json"

PROMPT_PREFIX = (
    "product photography, garment flat lay on plain white background, "
    "studio lighting, high quality, sharp focus, no model, "
)
...
def _make_name(prompt: str, uid: str) -> str:
    ...
    slug = "-".join(words[:4]) if words else "garment"
    return f"{slug}-{uid[:8]}"


class GarmentGenerator:
    """Generates garment product images from a text prompt."""
    ...
    async def generate(
        ...
        log.info("garment_generation_start")
        ...
        log.info("garment_generation_complete", image_url=image_url)
        ...
        log.info("garment_image_saved", file_path=file_path)
    ...
    def _save_image(self, image_bytes: bytes, item_id: str) -> str:
        ...
        file_path = GARMENT_OUTPUT_DIR / f"{item_id}.jpg"
    ...
    def _append_record(
        ...
        records: list = []
        if GARMENT_REGISTRY.exists():
            try:
                records = json.loads(GARMENT_REGISTRY.read_text(encoding="utf-8"))
